...
class _LoginPageState extends State<LoginPage> {
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              } else if (_loginState == MessageType.AUTH) ... {
                Text("Auth")
              },
              SizedBox(height: 60),
              Text('Messages'),
  ...
  void onMessage(Message message) async {
    ...
    _messages.addFirst("$_counter: ${message.toString()}");
    _counter++;
    ...
    if (types.contains(message.msg)) {
      if (message.msg == MessageType.ACCESS_RIGHTS) {
        ...
        if (auth.url != null) {
          ...
          final response = await http.get(Uri.parse(auth.url!));
          print(response.body);

          // Terminate Process (here removes the current view)
      ...
      if ((_loginState == MessageType.ENTER_PIN ||
          _loginState == MessageType.ENTER_NEW_PIN) &&
